package com.example.yuki.janken;
...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int myHand = 0;
        //インテントからデータを取り出す
        Intent intent = getIntent();//インテントを取得
        ...
        int gameResult = (comHand - myHand + 3) %3 ;//じゃんけん勝敗判定ロジックを活用
        switch (gameResult){
            case 0:
                //アイコの場合
...
private void saveData(int myHand, int comHand, int gameResult){
    ...
    int winningStreakCount = pref.getInt("WINBING_STREAK_COUNT", 0);
    int lastComHand = pref.getInt("Last_COM_HAND", 0);//コンピュータの前回の手
    int lastGameResult = pref.getInt("GAME_RESULT", -1);//前回の勝敗

    editor.putInt("GAME_COUNT", gameCount+1);
    if(lastGameResult == 2 && gameResult == 2) {//コンピュータが連勝した場合
